...
%% clear workspace
%% TODO detect whether the mice were on the platform for each shock or not
clearvars -except BpodSystem
close all
alignment = 'all';
event_labels = {'water', 'light', 'sound', 'laser', 'shock', 'shock received', 'shock avoided'};
bpod_key_word = 'Conflict';
analog_key_word = 'Analog';
behavior_data = 1;
signal_key_word = 'photo';
record_tracking_video = 0;
record_sample = 2;
fiber = 2;
fiber_names = {'TS', 'CeL'};
...
start_of_bpod_sec = centroid_time_scale(1);
...
new_time_spent = zeros(vidHeight,vidWidth);
sound_time_spent = zeros(vidHeight,vidWidth);
sound_off_time_spent = zeros(vidHeight,vidWidth);
copy_back = [];
frames_deleted = 0;
first_chunk = 0;
...
for i = 1:size(track_data,1)
    ...
    vert_num = round(track_data(i, 2) - top_left_corner(1));
    horz_num = round(track_data(i, 1) - top_left_corner(2));
    I2 = im2double(J);
    ...
    if and(vert_num + top_left_corner(1) < top_left_corner(1) + (bottom_left_corner(1) - top_left_corner(1))/2, vert_num > top_left_corner(1))
        %if in top half
        if and(horz_num + top_left_corner(2) > bottom_left_corner(2) + (bottom_right_corner(2) - bottom_left_corner(2))/2, horz_num < bottom_right_corner(2))
            %if in right half
    ...
    if and(vert_num + top_left_corner(1) > top_left_corner(1) + (bottom_left_corner(1) - top_left_corner(1))/2, vert_num < bottom_left_corner(1))
        %if in bottom half
        if and(horz_num + top_left_corner(2) < bottom_left_corner(2) + (bottom_right_corner(2) - bottom_left_corner(2))/2, horz_num < bottom_right_corner(2))
            %in left half
            %disp('In reward zone');
    ...
    if or(horz_num + top_left_corner(2) > top_right_corner(2), horz_num < 0)
        %delete this
        disp('deleted because out of frame');
        track_data(i, 2) = NaN;
        track_data(i, 1) = NaN;
        continue
    end
...
track_data(:, 1) = track_data(:, 1) - top_left_corner(2);
...
for q = 1:SessionData.nTrials
    ...
    if isfield(SessionData.RawEvents.Trial{1, q}.States, 'SoundOn')
        if ~isnan(SessionData.RawEvents.Trial{1, q}.States.SoundOn(1))
            stimulus_time = SessionData.RawEvents.Trial{1, q}.States.SoundOn;
            ...
            % Find if the mouse is on the platform
            if and(vert_num < top_left_corner(1) - 10 + (bottom_left_corner(1) - top_left_corner(1))/2, vert_num > top_left_corner(1))
                %if in top half
                if and(horz_num > 10 + bottom_left_corner(2) + (bottom_right_corner(2) - bottom_left_corner(2))/2, horz_num < bottom_right_corner(2))
                    %if in right half
    ...
    elseif isfield(SessionData.RawEvents.Trial{1, q}.States, 'SoundCueOn2')
        if ~isnan(SessionData.RawEvents.Trial{1, q}.States.SoundCueOn2(1))
            stimulus_time = SessionData.RawEvents.Trial{1, q}.States.SoundCueOn2;
            sound_on_time = stimulus_time(1);
            sound_off_time = stimulus_time(2);
            sound_on_times = horzcat(sound_on_times, sound_on_time + trial_start);% - session_start);
            sound_off_times = horzcat(sound_off_times, sound_off_time + trial_start);% - session_start);
            ...
            % Find if the mouse is on the platform
            if and(vert_num + top_left_corner(1) < top_left_corner(1) + (bottom_left_corner(1) - top_left_corner(1))/2, vert_num + top_left_corner(1) > top_left_corner(1))
                %if in top half
                if and(horz_num + top_left_corner(2) > bottom_left_corner(2) + (bottom_right_corner(2) - bottom_left_corner(2))/2, horz_num + top_left_corner(2) < bottom_right_corner(2))
                    %if in right half
                    shock_avoided_times = horzcat(shock_avoided_times, sound_on_time + 18 + trial_start);
                    shock_avoided_ind = horzcat(shock_avoided_ind, q);
                else
        ...
        if plot_heatmap_and_average
            ...
            if ~isempty(event_trial_ind)
                ...
                for r = 1:size(US_trials, 1)
                    cur = US_trials(r, :);
                    baseline = cur(1:((pre_time - 1)*10 - 5));
            ...
            if volume_to_check == 1000
                ...
                if ~isempty(event_trial_ind)
                    ...
                    elseif strcmp(alignment, 'right')
                        ...
                        save('dFF_data.mat', 'US_trials', 'US_target_frame', 'x_axis', 'baselines');
                    end
                end
...
%% Find the total distribution of location
for i = 1:size(track_data,1)
    ...
    elseif and(~isnan(track_data(i, 1)), first_chunk == 0)
        ...
        track_data(i,2) = track_data(i,2) + x_cropped_tracking_adjustment(round(track_data(i,2)));
        track_data(i,1) = track_data(i,1) + x_cropped_tracking_adjustment(round(track_data(i,1)));
        ...
        if ~isnan(track_data(i, 1))
            if or(round(track_data(i,2) - top_left_corner(1)) < 1, round(track_data(i,1) - top_left_corner(2)) < 1)
                track_data(i, 1) = NaN;
                track_data(i, 2) = NaN;
            end
        end
        ...
        elseif ~isempty(copy_back)
            ...
            for halp = copy_back
                y_time = round(track_data(halp,2) - top_left_corner(1));
                x_time = round(track_data(halp,1) - top_left_corner(2));
        ...
        if isempty(copy_back)
            new_time_spent(round(track_data(i,2) - top_left_corner(1)), round(track_data(i,1) - top_left_corner(2))) = new_time_spent(round(track_data(i,2) - top_left_corner(1)), round(track_data(i,1) - top_left_corner(2))) + 1/fps;
            %new_time_spent(round(track_data(i,2)), round(track_data(i,1))) = new_time_spent(round(track_data(i,2)), round(track_data(i,1))) + 1/fps;
        end
...
for k = 1:size(track_data,1)
    if and(track_data(k, 3) > sound_on_times_copy(1), track_data(k, 3) < sound_on_times_copy(1) + sound_duration)
        sound_on_data(size_sound, :) = track_data(k, :);
        size_sound = size_sound + 1;
    end
    if (sound_on_times_copy(1) + sound_duration) < track_data(k, 3)
...
for i = 1:size(sound_on_data,1)
    ...
    elseif ~isempty(copy_back)
        ...
        for halp = copy_back
            y_time = round(sound_on_data(halp,2) - top_left_corner(1));
            x_time = round(sound_on_data(halp,1) - top_left_corner(2));
    ...
    if isempty(copy_back)
        sound_time_spent(round(sound_on_data(i,2) - top_left_corner(1)), round(sound_on_data(i,1) - top_left_corner(2))) = sound_time_spent(round(sound_on_data(i,2) - top_left_corner(1)), round(sound_on_data(i,1) - top_left_corner(2))) + 1/fps;
    end
end 
...
for i = 1:size(sound_off_data,1)
    ...
    elseif ~isempty(copy_back)
        ...
        for halp = copy_back
            y_time = round(sound_off_data(halp,2) - top_left_corner(1));
            x_time = round(sound_off_data(halp,1) - top_left_corner(2));
    ...
    if isempty(copy_back)
        sound_off_time_spent(round(sound_off_data(i,2) - top_left_corner(1)), round(sound_off_data(i,1) - top_left_corner(2))) = sound_off_time_spent(round(sound_off_data(i,2) - top_left_corner(1)), round(sound_off_data(i,1) - top_left_corner(2))) + 1/fps;
    end
end 


%% Smooth the tracking data
smoothed_data_x = track_data(:,1) - top_left_corner(2);
smoothed_data_y = track_data(:,2) - top_left_corner(1);
...
%all time
rows = round(size(new_time_spent, 1)/2);
columns = round(size(new_time_spent, 2)/2);
...
%sound time
sound_rows = round(size(sound_time_spent, 1)/2);
sound_columns = round(size(sound_time_spent, 2)/2);
...
new_time_filter = bandpass_disk_filter(new_time_spent, 50, 1000);
sound_time_filter = bandpass_disk_filter(sound_time_spent, 50, 1000);
sound_off_time_filter = bandpass_disk_filter(sound_off_time_spent, 50, 1000);
...
corrected_sound_time_filter = bandpass_disk_filter(sound_time_spent .* sound_correction, 50, 1000);
corrected_sound_off_time_filter = bandpass_disk_filter(sound_off_time_spent .* sound_off_correction, 50, 1000);
...
subplot(2,3,5);
%%% TODO fix the heatmap
imagesc(corrected_sound_time_filter,[0 .04]); 
% colormap(gca,'jet');
box off; axis off; 
...
subplot(2,3,6);
%%% TODO fix the heatmap
imagesc(corrected_sound_off_time_filter,[0 .04]); 
% colormap(gca,'jet');
box off; axis off; 
...
if record_tracking_video
    ...
    while and(hasFrame(reader), p <= max_size)
        ...
        if p < start_of_bpod_frame
            p = p + 1;
            continue
        end
        I2 = im2double(img);
        centroid_y = round(smoothed_track_data(p, 1));
        centroid_x = round(smoothed_track_data(p, 2));
        if centroid_x < 4
            centroid_x = 4;
        end
        if centroid_y < 4
