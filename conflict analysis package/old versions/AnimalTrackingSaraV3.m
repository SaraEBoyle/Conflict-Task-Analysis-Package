...
event_labels = {'water', 'light', 'sound', 'laser', 'shock', 'shock received', 'shock avoided', 'sound in shock received', 'sound in shock avoided', 'platform entries', 'platform exits', 'sound platform entries', 'first platform entries', 'first platform exits after shock'};
...
flip_arena = 1; % just leave this
%disp('flip if water is on top right?')
fiber_names = {'TS', 'CeL'};

%% photometry variables
photometry_data = 1; %if there's photometry data change to 1
pre_time = 15; % time before events to plot
post_time = 15; %Time after
baseline_time = 5; %Time from beginning of trial to count as baseline
fiber = 2; %which fiber is the signal fiber
control_fiber = 2; %which is the control
...
for q = 1:min(SessionData.nTrials, size(start_times, 2))
    ...
    elseif isfield(SessionData.RawEvents.Trial{1, q}.States, 'SoundCueOn2')
        if ~isnan(SessionData.RawEvents.Trial{1, q}.States.SoundCueOn2(1))
            ...
            sound_on_time = stimulus_time(1);
            sound_off_time = stimulus_time(2);
            sound_on_times = horzcat(sound_on_times, sound_on_time + trial_start - sound_offset);% - session_start);
...
for s = 1:size(nose_poke_times, 2)
...
savefig('Behavior/nose_pokes.fig')
...
save('Behavior/session_performance', 'total_shocks_received', 'total_shocks_avoided', 'total_water_delivered')
...
%}
    ...
    subplot(2,3,5);
    %%% TODO fix the heatmap
    imagesc(corrected_sound_time_filter,[0 .15]); 
    axis('square')
    % colormap(gca,'jet');
    box off; axis off; 
    ...
    subplot(2,3,6);
    %%% TODO fix the heatmap
    imagesc(corrected_sound_off_time_filter,[0 .15]); 
    axis('square')
    % colormap(gca,'jet');
    box off; axis off; 
