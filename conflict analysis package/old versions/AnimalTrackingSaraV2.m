...
close all
alignment = 'all';
event_labels = {'water', 'light', 'sound', 'laser', 'shock', 'shock received', 'shock avoided', 'sound in shock received', 'sound in shock avoided'};
events_to_analyze = 1:9;
bpod_key_word = 'Conflict';
analog_key_word = 'Analog';
behavior_data = 1;
signal_key_word = 'photo';
record_tracking_video = 1;
record_sample = 2;
fiber = 2;
control_fiber = 2;
plot_location = 1;
...
if ~exist('nam', 'var') || isempty(nam)
    [file_nm, dir_nm] = uigetfile(fullfile('*.mov'));
    if dir_nm==0
...
x_ratio = 1 + (size(temp, 1) - size(J, 1))/size(temp, 1);
y_ratio = 1 + (size(temp, 2) - size(J, 2))/size(temp, 2);

track_data(:, 1) = track_data(:, 1) .* y_ratio;
track_data(:, 2) = track_data(:, 2) .* x_ratio;
...
for i = 1:size(track_data,1)
    ...
    if or(horz_num < 0, horz_num > round(rect2(4)))
    ...
    if or(vert_num < 0, vert_num > round(rect2(3)))
        %delete this
...
for q = 1:SessionData.nTrials
    ...
    if isfield(SessionData.RawEvents.Trial{1, q}.States, 'SoundOn')
        if ~isnan(SessionData.RawEvents.Trial{1, q}.States.SoundOn(1))
            ...
            % Find if the mouse is on the platform
            if and(vert_num + top_left_corner(1) < top_left_corner(1) - 10 + (bottom_left_corner(1) - top_left_corner(1))/2, vert_num + top_left_corner(1) > top_left_corner(1))
                %if in top half
                if and(horz_num + top_left_corner(2) > 10 + bottom_left_corner(2) + (bottom_right_corner(2) - bottom_left_corner(2))/2, horz_num + top_left_corner(2) < bottom_right_corner(2))
                    %if in right half
        ...
        for event_type_ind = events_to_analyze
                ...
                if volume_to_check == 1000
                    ...
                    if ~isempty(event_trial_ind)
                        ...
                        elseif strcmp(alignment, 'right')
                            ...
                            save('dFF_data.mat', 'US_trials', 'US_target_frame', 'x_axis', 'baselines', 'auto_flo');
                        end
                    end
            ...
            %                    save('z_scores.mat', 'z_US_trials', 'z_no_US_trials', 'x_axis');
                    ...
                    elseif strcmp(alignment, 'left')
                        ...
                        save('dFF_data.mat', 'US_trials', 'no_US_trials', 'US_target_frame', 'CS_target_frame', 'x_axis', 'baselines', 'auto_flo');
                    end
