...
% ---> Inputs: .mov video of the session, .png background photo without
% mouse
...
function [] = AnimalTrackingSaraV7(bleaching_correction, event_groups, pre_times, post_times, baseline_times, control_fiber, signal_fibers, fiber_names, photometry_data, key_words)
...
if photometry_data == 1
    [start_times, photo_data, SessionData, track_data] = load_data(bpod_key_word, signal_key_word, analog_key_word, file, photometry_data);
end
...
for p = 2:size(platform_matrix, 1)
    ...
    if cur > prev
        ...
        pre = sum(platform_matrix((p - 10):(p - 1), 1));
        if post >= 8
...
for q = 1:min(SessionData.nTrials, size(start_times, 2))
    ...
    if isfield(SessionData.RawEvents.Trial{1, q}.Events, 'Port2In')
        if ~isnan(eval(horzcat('SessionData.RawEvents.Trial{1, q}.Events.', 'Port2In', '(1)')))
            ...
            poke_in_times = horzcat(poke_in_times, poke_in_time + trial_start);
            poke_in_dict_times{q} = poke_in_time + trial_start;
            if isfield(SessionData.RawEvents.Trial{1, q}.Events, 'Port2Out')
                poke_out_time = eval(horzcat('SessionData.RawEvents.Trial{1, q}.Events.', 'Port2Out'));
            else
                poke_out_time = [];
            end
            poke_out_times = horzcat(poke_out_times, poke_out_time + trial_start);
            poke_out_dict_times{q} = poke_out_time + trial_start;
            if extra_poke_in ~= 0
                start_dif = start_times(q) - start_times(q - 1);
                poke_in_time = horzcat(extra_poke_in - start_dif, poke_in_time);
                extra_poke_in = 0;
            end
            if extra_poke_out ~= 0
                start_dif = start_times(q) - start_times(q - 1);
                poke_out_time = horzcat(extra_poke_out - start_dif, poke_out_time);
                extra_poke_out = 0;
            end
            if ~isnan(poke_in_time)  
                if isempty(poke_out_time)
                    poke_in_time = [];
                elseif size(poke_in_time, 2) > size(poke_out_time, 2)
                    disp('too few pokes out')
                    if poke_in_time(1) > poke_out_time(1)
                        %if first poke in is before first poke out
                        %poke_in_time(1) = [];
                        disp('this should not be the case');
                    else
                        %the first poke in is smallest, which makes sense
                        extra_poke_in = poke_in_time(end);
                        poke_in_time(end) = [];
                        disp('added last poke in time to next trial');
                    end
                elseif size(poke_in_time, 2) < size(poke_out_time, 2)
                    disp('too few pokes in')
                    if poke_in_time(1) > poke_out_time(1)
                        if poke_in_time(1) - poke_out_time(1) > 10
                            disp('First lick over 10 seconds too early')
                            poke_out_time(1) = [];
                        ...
                        disp('The first out happened before the first in- this is a bpod glitch. Switch the numbers')
                        out_temp = poke_out_time(1);
                        in_temp = poke_in_time(1);
                        poke_out_time(1) = in_temp;
                        poke_in_time(1) = out_temp;
                        %poke_out_time(1) = [];
                        disp('switched first pokes');
                        if poke_in_time(end) < poke_out_time(end)
                            extra_poke_out = poke_out_time(end);
                            poke_out_time(end) = [];
                            disp('added last poke out time to next trial');
    ...
    if isfield(SessionData.RawEvents.Trial{1, q}.States, sound_state)
        if ~isnan(eval(horzcat('SessionData.RawEvents.Trial{1, ', num2str(q), '}.States.', sound_state, '(1)')))
            ...
            big_shock_pos = find(track_data(:,3) >= shock_on);
            big_shock_pos = track_data(big_shock_pos, :);
            small_shock_pos = find(big_shock_pos(:,3) <= (shock_on + shock_length));
            all_shock_pos = big_shock_pos(small_shock_pos, :);
            
%            first_shock_pos = small_shock_pos(small_shock_pos, :);
            off_plat = 0; %change to 1 if mouse leaves platform during shock
            for first_shock_ind = 1:size(all_shock_pos, 1)
                first_shock_pos = all_shock_pos(first_shock_ind, :);
...
%Attempt 3
%% Works the best. Add 8 previous second pokes to each trial so you have similar time frames of comparison
%Use poke in dict and poke out dicts
poke_duration_in_sound = [];
poke_duration_not_in_sound = [];
sound_pokes = 0;
no_sound_pokes = 0;
for num = 1:size(poke_in_dict_times, 2)
    ins = poke_in_dict_times{num};
    outs = poke_out_dict_times{num};
    if ~exist(horzcat('SessionData.RawEvents.Trial{1, ', num2str(num), '}.States.', sound_state))
        disp('no');
        sound_state = 'SoundCueOn';
    end
    if ~isnan(eval(horzcat('SessionData.RawEvents.Trial{1, ', num2str(num), '}.States.', sound_state, '(1)')))
        stimulus_time = eval(horzcat('SessionData.RawEvents.Trial{1, ', num2str(num), '}.States.', sound_state));
        stimulus_time = stimulus_time(1) + start_times(num);
        if num ~= 1 %% CHANGED IF BECAUSE OLD RUNS DONT HAVE ITI
            %end_prev_ITI = SessionData.RawEvents.Trial{1, num - 1}.States.ITI(2) + start_times(num - 1);
        else
            %end_prev_ITI = start_times(num);
            %stimulus_time = stimulus_time(1);
        end
    end
    %Get pokes in during sound
    if ~isempty(ins)
        ins_during_sound = find(ins > stimulus_time);
    else
        ins_during_sound = [];
    end
    ins_during_sound = ins(ins_during_sound);
    before_end = find(ins_during_sound < (stimulus_time + 20));
    ins_during_sound = ins_during_sound(before_end);
    
    %Get pokes out during sound
    if ~isempty(outs)
        outs_during_sound = find(outs > stimulus_time);
    else
        outs_during_sound = [];
    end
    outs_during_sound = outs(outs_during_sound);
    before_end = find(outs_during_sound < (stimulus_time + 20));
    outs_during_sound = outs_during_sound(before_end);
    
    if size(outs_during_sound, 2) > size(ins_during_sound, 2)
        %If too many outs
        if isempty(ins_during_sound)
            ins_during_sound = horzcat(stimulus_time(1), ins_during_sound);
            disp('added stimulus time to beginning of (empty) ins during sound')
        elseif outs_during_sound(1) < ins_during_sound(1)
            %If first out is too little (poke in was before sound)
            %Add poke in at time of stimulus
            ins_during_sound = horzcat(stimulus_time(1), ins_during_sound);
            disp('added stimulus time to beginning of ins during sound')
        elseif outs_during_sound(end) < ins_during_sound(end)
            %last out smaller than last in
            outs_during_sound(end) = [];
            disp('deleted last out during sound')
            
        end
    elseif size(outs_during_sound, 2) < size(ins_during_sound, 2)
        %more ins than outs
        if isempty(outs_during_sound)
            disp('added end of stimulus to the end of (empty) outs during sound');
            outs_during_sound = horzcat(outs_during_sound, (stimulus_time(1) + 20));
        elseif ins_during_sound(end) > outs_during_sound(end)
            %if last in is bigger than last out add stim time end to end of
            %outs
            outs_during_sound = horzcat(outs_during_sound, (stimulus_time(1) + 20));
            disp('added end of stimulus to the end of outs during sound')
        else
            disp('what do');
    ...
    sound_pokes = sound_pokes + size(outs_during_sound, 2);  
    poke_duration_in_sound = horzcat(poke_duration_in_sound, sum(outs_during_sound - ins_during_sound));
    
    % Get pokes during no sound
    %Get pokes in during no sound
    if num ~= 1
        %Find pokes in last 8 seconds before current trial
        prev_ins_ind = find(prev_ins > ((stimulus_time - 20))); %used to be end_prev_ITI - 8
        prev_ins = prev_ins(prev_ins_ind);
        prev_outs_ind = find(prev_outs > ((stimulus_time - 20))); %used to be end_prev_ITI - 8
        prev_outs = prev_outs(prev_outs_ind);
        %If there's a preblem in the beginning, horzcatting messes
        %everything up
        if ~isempty(ins)
            first_in = ins(1);
        else
        ...
        if and(first_in > first_out, size(outs, 2) > size(ins, 2))
            if size(prev_ins, 2) <= size(prev_outs, 2)
                outs(1) = [];
            end
        end
        
        ins = horzcat(prev_ins, ins);
        outs = horzcat(prev_outs, outs);
    end
    %get ins not during sound
    ins_not_during_sound = find(ins < stimulus_time);
    ins_not_during_sound = ins(ins_not_during_sound);
    if ~isempty(ins_not_during_sound)
        before_end = find(ins_not_during_sound > (stimulus_time - 20)); %used to be 0
    else
        before_end = []; %used to be 0
    end
    ins_not_during_sound = ins_not_during_sound(before_end);
    
    %Get pokes out not during sound
    outs_not_during_sound = find(outs < stimulus_time);
    outs_not_during_sound = outs(outs_not_during_sound);
    if ~isempty(outs_not_during_sound)
        before_end = find(outs_not_during_sound > (stimulus_time - 20)); %used to be 0
    else
        before_end = [];
    end
    outs_not_during_sound = outs_not_during_sound(before_end);
    
    %If the poke in starts before the threshold say poke in is the
    %threshold
    if size(outs_not_during_sound, 2) > size(ins_not_during_sound, 2)
        %If too many outs
        if isempty(ins_not_during_sound)
            ins_not_during_sound = horzcat(((stimulus_time - 20)), ins_not_during_sound); %Used to beend_prev_ITI - 8
            disp('Added previous ITI - 8 to beginning of (empty) ins not during sound');
        elseif outs_not_during_sound(1) < ins_not_during_sound(1)
            %If first out is too little (poke in was before sound)
            %Add poke in at time of trial start
            if num ~= 1
                ins_not_during_sound = horzcat(((stimulus_time - 20)), ins_not_during_sound); %used to be end_prev_ITI - 8
            else
                if (stimulus_time - 20) < 0
                    ins_not_during_sound = horzcat(0, ins_not_during_sound);
                else
                    ins_not_during_sound = horzcat((stimulus_time - 20), ins_not_during_sound);
                end
            end
            disp('Added previous ITI - 8 to beginning of ins not during sound');
        else
            disp('what do');
            diffies = outs_not_during_sound(1:(size(ins_not_during_sound, 2))) - ins_not_during_sound;
            %if outs_not_during_sound(end) > 
        end
    elseif size(outs_not_during_sound, 2) < size(ins_not_during_sound, 2)
        %more ins than outs
        if isempty(outs_not_during_sound)
            outs_not_during_sound = horzcat(outs_not_during_sound, stimulus_time(1));
            disp('added stimulus time to end of (empty) outs not during sound')
        elseif ins_not_during_sound(end) > outs_not_during_sound(end)
            %if last in is bigger than last out add stim time to end of
            %outs
            outs_not_during_sound = horzcat(outs_not_during_sound, stimulus_time(1));
            disp('added stimulus time to end of outs not during sound')
        else
            disp('add stimulus time to out not during sound');
            outs_not_during_sound = horzcat(outs_not_during_sound, stimulus_time(1));
            diffies = outs_not_during_sound - ins_not_during_sound;
        end
    end
    no_sound_pokes = no_sound_pokes + size(outs_not_during_sound, 2);
    if size(ins_not_during_sound, 2) > size(outs_not_during_sound, 2)
        if ins_not_during_sound(1) < outs_not_during_sound(1)
            ins_not_during_sound(end) = [];
        end
    end
    if size(outs_not_during_sound, 2) ~= size(ins_not_during_sound, 2)
        if size(outs_not_during_sound, 2) > size(ins_not_during_sound, 2)
            if outs_not_during_sound(1) >ins_not_during_sound(1)
                outs_not_during_sound(end) = [];
            else
                disp('what do');
            end
        else
            disp('ins and outs are not the same size. Something is super wrong');
        end
        
    end
    poke_duration_not_in_sound = horzcat(poke_duration_not_in_sound, sum(outs_not_during_sound - ins_not_during_sound));
    if poke_duration_not_in_sound(end) < 0   
        disp('why is this negative');
        if ins_not_during_sound(1) > outs_not_during_sound(1)
            if num ~= 1
                ins_not_during_sound = horzcat(((stimulus_time - 20)), ins_not_during_sound); %end_prev_ITI - 8 
            else
                if (stimulus_time - 20) < 0
                    ins_not_during_sound = horzcat(0, ins_not_during_sound);
                else
            ...
            ins_not_during_sound = ins_not_during_sound(1:(end-1));
            poke_duration_not_in_sound(end) = [];
            if sum(outs_not_during_sound - ins_not_during_sound) > 0
                disp('resolved negative issue');
            end
            poke_duration_not_in_sound = horzcat(poke_duration_not_in_sound, sum(outs_not_during_sound - ins_not_during_sound));
        end
    ...
    if poke_duration_not_in_sound(end) > 20
        %20 because that's the amount before the sound
        diffies = outs_not_during_sound - ins_not_during_sound;
        poke_duration_not_in_sound(end) = [];
        disp('Too much poking time in no sound, deleted trial');
    end
    if poke_duration_in_sound(end) < 0
        if ins_during_sound(1) > outs_during_sound(1)
           ins_during_sound = horzcat(stimulus_time, ins_during_sound); 
           ins_during_sound(end) = [];
           diffies = outs_during_sound - ins_during_sound;
           if sum(diffies) < 0
               error('why is this negative');
           else
               poke_duration_in_sound(end) = sum(diffies);
           end
        else
            disp('negative but dont know why')
        end
        
    end
    if poke_duration_in_sound(end) > 20
        %20 because that's the amount of the sound
        error('Too much poking time in sound');
    end
    prev_ins = ins;
    prev_outs = outs;
end
...
%Attempt 6- 2 lists, start with one and go for the first out after the in
%check if mouse is in correct corner
...
sound_on_nose_pokes = sound_pokes;
sound_off_nose_pokes = no_sound_pokes;
...
%{
% TODO- make it so pokes spanning sound on and off are calculated correctly
% did sound on to sound off, still need to do sound off to sound on
for s = 1:size(poke_times, 2)
    if and(nose_poke_times(1, s) > sound_on_times_copy(1), nose_poke_times(1, s) < sound_on_times_copy(1) + sound_duration - sound_offset)
        %If poke in after sound but before end of sound
        sound_on_nose_pokes = sound_on_nose_pokes + 1;
        if and(poke_times(2, s) > sound_on_times_copy(1), poke_times(2, s) < sound_on_times_copy(1) + sound_duration - sound_offset)
            %if poke out is within sound
            %sound_on_poke_time = sound_on_poke_time + abs(poke_times(2, s) - poke_times(1, s));
        else
            %if poke extends beyond sound, cut it off at end of sound
            %sound_on_poke_time = sound_on_poke_time + abs(poke_times(2, s) - (sound_on_times_copy(1) + sound_duration - sound_offset));
        end
    else
        %If not in sound window
        ITI_end = SessionData.RawEvents.Trial{1, manual_trial_num}.States.ITI(2);
        if and((start_times(manual_trial_num) + ITI_end - 8 - sound_offset) < nose_poke_times(1, s), nose_poke_times(1, s) < ITI_end)
            %If after end of sound and in last 8 seconds of the ITI
        ...
        elseif and((sound_on_times_copy(1) - sound_offset) > nose_poke_times(1, s), nose_poke_times(1, s) > start_times(manual_trial_num))
    ...
    if (start_times(manual_trial_num) + ITI_end) < nose_poke_times(1, s)
        %If after end of sound
        sound_on_times_copy(1) = [];
        manual_trial_num = manual_trial_num + 1;
        if isempty(sound_on_times_copy)
            break
        end
    end
...
total_sound_off_time = total_sound_time;
if ~exist('Behavior')
    mkdir('Behavior');
end

%plot number of pokes
%% TODO the nose pokes are in the wrong time period. whole session, not just 20 seconds before sound
X = categorical({'Total', 'Sound Off', 'Sound On'});
X = reordercats(X, {'Total', 'Sound Off', 'Sound On'});
Y = [(sound_off_nose_pokes + sound_on_nose_pokes)/(total_sound_time + total_sound_off_time), sound_off_nose_pokes/total_sound_off_time, sound_on_nose_pokes/total_sound_time];
bar(X, Y);
title('Number of nose pokes per second');
savefig('Behavior/number_of_nose_pokes.fig')

%Plot time spent poking
mean_poke_duration_not_in_sound = mean(poke_duration_not_in_sound);
mean_poke_duration_in_sound = mean(poke_duration_in_sound);
X = categorical({'Total', 'Sound Off', 'Sound On'});
X = reordercats(X, {'Total', 'Sound Off', 'Sound On'});
all_poke_time = sum(poke_duration_in_sound) + sum(poke_duration_not_in_sound);
c_all_poke_time = all_poke_time/(total_sound_time + total_sound_off_time);
c_sound_off_poke_time = sum(poke_duration_not_in_sound)/total_sound_off_time;
c_sound_on_poke_time = sum(poke_duration_in_sound)/total_sound_time;
Y = [all_poke_time, sum(poke_duration_not_in_sound), sum(poke_duration_in_sound)];
bar(X, Y);
title('Nose poke duration');
savefig('Behavior/nose_poke_duration.fig')
save('Behavior/nose_poke_duration.mat', 'all_poke_time', 'poke_duration_in_sound', 'poke_duration_not_in_sound', 'mean_poke_duration_in_sound', 'mean_poke_duration_not_in_sound');
...
total_water_deliveries = size(water_delivered_times, 1);
save('Behavior/session_performance', 'total_shocks_received', 'total_shocks_avoided', 'total_water_deliveries', 'poke_duration_in_sound', 'poke_duration_not_in_sound', 'mean_poke_duration_in_sound', 'mean_poke_duration_not_in_sound')
